...
export default async function OnboardingPage({
  ...
  const { data: profile } = await supabase!
    ...
    .select(
      "onboarding_completed_at, profession, service_niche, service_name, booking_mode, stage, current_bottleneck, channels, opportunity_signals, daily_available_minutes, can_serve_next_7_days, has_service_proof, has_real_portfolio, has_booking_path, proof_type"
    )
    .eq("user_id", userId)
    .maybeSingle();
  ...
  const initialValue = profile
    ? {
        ...
        proofType:
          profile.proof_type ??
          (profile.has_service_proof || profile.has_real_portfolio
            ? "portfolio"
            : "none"),
        stage: profile.stage,
        ...
        hasServiceProof:
          profile.has_service_proof ?? profile.has_real_portfolio ?? false,
  ...
  return (
    <OnboardingForm
      ...
      reviewNotice={params.reason === "moment" || params.reason === "edit"}
    />
  );
}
