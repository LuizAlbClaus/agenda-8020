...
export default function BenefitCard({ benefit, handoff = false, integrationStatus = "not_configured", publicUrl, loginUrl = "https://belevy.com.br/login" }: { benefit: BelevyBenefit; handoff?: boolean; integrationStatus?: IntegrationStatus; publicUrl?: string; loginUrl?: string }) {
  ...
  function activate() {
    setMessage(""); setError(false); setAccessEmailFailed(false);
    startTransition(async () => {
      ...
      if (!result.ok) {
        ...
        setMessage("Benefício ativado. Verifique seu e-mail para acessar o Belevy.");
      }
    });
  }
  ...
  return (
    <section className={`rounded-2xl border p-5 shadow-sm ${handoff ? "border-teal-200 bg-teal-50" : "border-slate-200 bg-white"}`} aria-labelledby="belevy-heading">
      ...
      {message && <p role={error ? "alert" : "status"} className={`mt-3 text-sm ${error ? "text-red-700" : "text-teal-800"}`}>{message}{accessEmailFailed && <>{" "}<a href={loginUrl} target="_blank" rel="noreferrer" className="font-semibold underline underline-offset-4">Abrir login do Belevy</a></>}</p>}
    </section>
  );
}
