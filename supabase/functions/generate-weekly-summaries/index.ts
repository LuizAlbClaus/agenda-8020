...
if (!supabaseUrl || !serviceRoleKey) {
  throw new Error("weekly_summaries_configuration_missing");
}
...
Deno.serve(observedHandler("generate-weekly-summaries", async (request, context) => {
  ...
  try {
    ...
    // Empty POST uses the database timezone-aware default.
  }
  setEntity(context, "scheduled_job", "weekly_summaries");

  const { data, error } = await supabase.rpc("enqueue_weekly_summaries", {
    p_limit: limit,
  });
  if (error) {
    setErrorCode(context, "GENERATION_FAILED");
    console.error("weekly_summaries_generation_failed", { code: error.code });
